...
void addTopologyNode(CfgEditor &editor, split_zds_trajectory_t* connector, size_t i)
{
    ...
    if (!bwd_side.isEmpty())
    {
        ...
        flist.append(QPair<QString, QString>("state_bwd", QString("%1").arg(connector->state_bwd)));
    }
    ...
    if (!fwd_side.isEmpty())
    {
        ...
        flist.append(QPair<QString, QString>("state_fwd", QString("%1").arg(connector->state_fwd)));
    }
